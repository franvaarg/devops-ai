...
function addContinuationHeader(doc: jsPDF) {
  ...
  doc.text("DEVOPS AI", PAGE_MARGIN, 11.5);
...
function addPageNumbers(doc: jsPDF) {
  ...
  for (
    ...
    doc.text(
      `Generated by DevOps AI | Page ${pageNumber} of ${totalPages}`,
      105,
      290,
      {
        align: "center",
...
export function exportAnalysisToPdf(
  ...
  doc.setProperties({
    title: `DevOps AI Analysis Report ${analysis.id}`,
  ...
  doc.setFontSize(22);
  doc.setTextColor(255, 255, 255);
  doc.text("DEVOPS AI", PAGE_MARGIN, 20);
  ...
  doc.save(
    `DevOpsAI_Analysis_${analysis.id}_${fileDate}.pdf`
  );
}